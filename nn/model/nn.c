...
struct Matrix forward(struct NeuralNet nn, struct Matrix inputs)
{
  ...
  for (int i = 0; i < nn.n_hidden_layers; i++)
  {
    ...
    matMul(&weights_matrix, &output, &output);
...
{
  struct NeuralNet nn = {
      .n_input = n_inputs, .n_hidden_layers = n_layers, .n_output = n_outputs};
  // inputs in vectors, n_input => n_input x 1 vector, weight layers n_input x
  // n_input, bias vector n_output_from_layers x 1
